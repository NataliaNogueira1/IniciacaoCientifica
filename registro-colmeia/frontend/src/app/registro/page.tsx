...
export default function Home() {
  ...
  const [colmeias, setColmeias]       = useState<ColmeiaInfo[]>([]);
  const [localizacao, setLocalizacao] = useState<ColmeiaInfo | null>(null);
  ...
  // Carrega colmeias do banco (só as ativas — sem prefixo [INATIVA])
  useEffect(() => {
    fetch("/api/publico/colmeias")
      .then((r) => r.ok ? r.json() : [])
      .then((data: { nome: string; cidade: string; latitude?: number; longitude?: number; altitude?: number }[]) => {
        setColmeias(
          data
            .filter((c) => !c.nome.startsWith("[INATIVA]"))
            .map((c) => ({
              nome: c.nome,
              cidade: c.cidade ?? "",
  ...
  useEffect(() => {
    const found = colmeias.find((c) => c.nome === form.colmeia) ?? null;
    setLocalizacao(found);
  }, [form.colmeia, colmeias]);
  ...
  return (
    ...
    >
      ...
      <div className="sm:contents">
        <TopBar user={user} active="registro" onLogout={handleLogout} />
      </div>

      <div className="max-w-5xl mx-auto flex flex-col gap-4 sm:gap-6 mt-4 sm:mt-0 sm:pt-24">
        {/* Título */}
        <div className="w-full bg-yellow-950 rounded-2xl shadow-xl px-6 py-7 flex items-start gap-4">
          <div className="-mt-1">
            <BeeIcon size="1.6rem" />
          </div>
              ...
              <fieldset className="flex flex-col gap-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <SelectField
                    ...
                    options={colmeias.map((c) => ({ value: c.nome, label: c.nome }))}
                  />
                  <InputText
                    label="Data e hora"
                    id="dataHora"
